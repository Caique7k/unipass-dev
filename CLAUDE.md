...
- **`Trip`** (viagem) e o `EventType.LEAVING`: existem no schema e são referenciados em queries de relatório e no seed, mas **nenhum serviço cria um `Trip` ou emite um evento `LEAVING`** hoje. Desde 2026-09-22 o `getMetrics` do dashboard **não usa mais `Trip`** (ver seção 15), então a home não mostra mais contadores falsos; os **relatórios** (`getReport`/`buildFleetReport`) ainda leem `Trip` e continuam zerados em dados reais.
...
- **`Trip`/`LEAVING` mortos no código, vivos no schema**: risco de relatórios "mentirem" (mostrar zero viagens) sem deixar claro que a feature não está implementada. Já corrigido na home do dashboard (não depende mais de `Trip`); **ainda pendente nos relatórios** (`reportType: 'fleet'`).
...
- **2026-09-23 (paginação + modais)**:
  - **Paginação no backend estava quebrada e inconsistente.** Testado endpoint a endpoint contra a API: `/students` e `/devices` devolviam **HTTP 500** para `page=0`, `page=-1` e `page=abc` (skip negativo no Prisma); `/groups`, `/users` e `/billing/templates` engoliam lixo calado e **aceitavam `limit=999999`** (sem teto); só `/buses` e `/routes` validavam de verdade. Criado **`backend/src/common/dto/pagination-query.dto.ts`** (`PaginationQueryDto`, `MAX_PAGE_SIZE = 1000`) e todos os DTOs de listagem passaram a herdar dele: `find-groups`, `find-billing-templates`, `find-devices`, `find-buses` e os novos `find-students` e `find-users`. Os controllers de students/users deixaram de ler `@Query('page')` na mão. Resultado verificado: todos os 7 endpoints devolvem **400 com mensagem** para entrada inválida e `limit=1001` é recusado; `limit=1000` continua valendo porque os combos do cadastro de aluno dependem disso.
  - **Paginação no frontend**: `DataTable` ganhou salto para **primeira/última página** e reticências (antes, com 50 páginas, não havia como chegar ao fim sem clicar 48 vezes). A janela foi conferida caso a caso (1/1, 1/50, 25/50, 50/50, 1/2…) — sempre mostra primeira e última, sem número repetido. `useListQuery` ganhou `onPageOutOfRange`: se a página pedida deixou de existir (último item da página foi desativado), volta sozinha para a última página válida em vez de mostrar tabela vazia. Ligado nas 8 telas de lista.
  - **Modais redesenhados.** `components/FormModal.tsx` refeito: ícone no cabeçalho, botão de fechar, `banner` opcional, corpo rolável, rodapé fixo, 4 tamanhos, `FormSection`/`FormField` e `ModalCancelButton`/`ModalSubmitButton` (com estado de carregamento). **Os 8 modais que ainda usavam `Dialog` cru foram migrados**: grupos, rotas, grupos de boletos, UniHub (pareamento), "como parear", usuários, horários e alunos. Em todos, só a moldura mudou — validação, máscaras e envio seguem iguais.
  - **Modal de exclusão** virou `components/ConfirmDialog.tsx` (saiu do `page-kit`, que passou a reexportá-lo): ícone de alerta, título que conta os itens, **lista dos registros que serão afetados** (`items`, com "e mais N"), linha de consequência na cor da ação e botões distintos. Cada tela agora passa os nomes reais e uma consequência específica — ex.: em alunos, "A TAG dele deixa de ser aceita no embarque: a próxima leitura vira um evento negado".
  - **Observado durante os testes**: a captura de tela via CDP travou duas vezes numa aba antiga, mas a página seguia respondendo a JS e nada foi desativado no banco — era a aba/extensão, não o app. Reaberta a aba, tudo normal.
  - **Correções de texto**: acentos faltando no conteúdo dos modais de horário e de aluno ("Antecedencia", "Horario", "notificacao", "Edicao", "cartao", "codigo").

- **2026-09-22 (telas internas v2 — design system + performance)**: todas as telas do painel refeitas sobre um kit comum, **menos `/dashboard/location`** (fica para a próxima sessão).
  - **Kit novo** em `frontend/src/app/dashboard/`: `components/page-kit.tsx` (PageHeader, Toolbar, SearchField, FilterChips, PrimaryButton/GhostButton, StatusBadge, ConfirmDialog, ErrorState, FetchingBar), `components/DataTable.tsx` (colunas declarativas, seleção em massa, colunas responsivas via `hideBelow`, skeleton, estado vazio e paginação) e `components/FormModal.tsx` (casca dos formulários + `FormField`). `nav.ts` já centralizava a navegação desde a sessão anterior.
  - **Três bugs de lista corrigidos de uma vez** (afetavam ônibus, grupos, usuários, dispositivos, rotas, horários, grupos de boletos e alunos): (1) a página inteira desmontava a cada busca (`if (loading) return <Skeleton/>` + `setLoading(true)` em todo fetch), então **o input perdia o foco a cada tecla**; agora `loading` é só o primeiro carregamento e as recargas usam `isFetching`; (2) **uma requisição por tecla** — agora há debounce de 350 ms (`hooks/useDebouncedValue.ts`); medido no navegador: 6 caracteres digitados = **1 requisição** (antes 6); (3) sem cancelamento nem proteção de ordem — `hooks/useListQuery.ts` aborta a consulta anterior (`AbortController`) e descarta respostas fora de ordem.
  - **Modal de aluno**: as três listas de opções (grupos, rotas, grupos de boletos, cada uma com `limit=1000`) eram rebuscadas **a cada abertura**. `students/hooks/useStudentFormOptions.ts` carrega uma vez e reaproveita; medido: reabrir o modal agora faz **0 requisições**.
  - **Regra financeira unificada**: o painel contava "vencidos" como `OVERDUE + (ISSUED|SENT vencidas)` = 4, enquanto `/dashboard/billing` contava `OVERDUE + (DRAFT|SCHEDULED|ISSUED|SENT vencidas)` = 5 — duas telas discordando do mesmo número. As regras saíram de `billing.service.ts` para **`backend/src/billing/billing-charge-status.util.ts`** (`buildOverdueChargeWhere`, `OPEN_CHARGE_STATUSES`, `CLOSED_CHARGE_STATUSES`) e o `dashboard.service.ts` passou a importar de lá. Agora as duas mostram 5. No painel o card foi renomeado para **"A vencer"**, porque "Em aberto" na tela de boletos inclui as vencidas (a vencer + vencidos = em aberto).
  - **Guarda nova (mudança de comportamento, intencional)**: em `/dashboard/users` o próprio usuário logado não pode mais ser selecionado para desativação. O backend (`UsersService.deactivateMany`) **não** impede isso — um ADMIN conseguia se trancar para fora da própria empresa. A guarda é só no frontend; o backend continua aceitando.
  - **Telas grandes (`reports`, `billing`)**: restilizadas de forma cirúrgica (cabeçalho → `PageHeader`, raios e superfícies normalizados para os tokens do tema, cores hex fixas removidas) **sem tocar na lógica de filtros, exportação em PDF ou emissão em lote**.
  - **Removidos** (código morto após a migração): `*Table.tsx` e `DeleteDialog.tsx` de buses/groups/users/devices/routes/route-schedules/billing-groups/students, e os hooks `useBuses`, `useGroups`, `useDevices`, `useRoutes`, `useSchedules`, `useUsers`, `useBillingGroups`, `useStudents`. Tipos que moravam nesses hooks foram para `types/` (`users/types/user.ts`, `students/types/student.ts`).
  - **Verificado no navegador**: foco preservado na busca, contagem de requisições, estados vazios, filtros, seleção em massa, checkbox da própria conta desabilitado, e o horário da rota lido em **UTC** (`17:30Z` renderiza 17:30 e não 14:30 — `getUTCHours` preservado de propósito).
  - **Pendências de escala anotadas, não resolvidas**: `GET /companies` devolve **todas** as empresas sem paginação (a busca da tela é client-side); `/transport/boarding-overview/today` devolve todos os alunos do dia e filtra no cliente; buscas usam `contains`/`insensitive` sem índice trigram. Nenhuma incomoda no tamanho atual, todas incomodam com escala.
  - **Inconsistência pré-existente observada**: `/dashboard/company` mostra "Alunos 3" (`_count.students` conta inativos) enquanto o painel mostra 2 alunos ativos. Não alterado.

- **2026-09-22 (dashboard v2 — shell + home)**: redesenho do painel com correção das métricas na origem.
  - **Backend (`dashboard.service.ts`)**: `getMetrics` reescrito. Antes, `tripsToday` e `busCapacityUsed` liam `Trip` — tabela que nenhum serviço popula — então dois dos quatro KPIs eram sempre `0`, e o gráfico "Entradas por Ônibus" era uma linha reta em zero. Agora tudo deriva de tabelas com dados reais: `TransportEvent` (embarques/desembarques/negados e alunos distintos), `Bus.capacity` (ocupação), `Device.lastUpdate` (online em 45s, mesma regra do `LocationService`), `RouteSchedule` (horários de hoje pelo `dayOfWeeks`), `NotificationPrompt`/`ScheduleConfirmation` (confirmações do dia) e `BillingCharge` (financeiro, **só para `ADMIN`** — `getMetrics(companyId, role)`; o controller passa `req.user.role`). A janela de 7 dias virou **uma** query (`findMany` + bucket por timezone) no lugar de 14 `count` em loop. O payload mudou de forma (novo formato em `frontend/src/app/dashboard/types/dashboard.ts`) e inclui `alerts[]` derivados (UniHub sem sinal, ônibus sem dispositivo, leituras negadas, notificações falhas, boletos vencidos). Alertas usam a chave `key` (não `id`) de propósito, para não passarem pelo `OpaqueIdResponseInterceptor`.
  - **Shell**: `nav.ts` novo centraliza a navegação e as regras de papel (antes duplicadas dentro da `Sidebar`). `Sidebar.tsx` refeita (grupos colapsáveis, indicador ativo com `layoutId`, largura animada, modo estreito com tooltip). `Topbar.tsx` refeita (trilha UniPass › grupo › página, busca `Ctrl/Cmd+K`, toggle de tema animado). `CommandPalette.tsx` novo (busca sem acento, navegação por setas/Enter). `layout.tsx` reorganizado: sidebar em altura total à esquerda, topbar dentro da coluna de conteúdo, transição por rota.
  - **Home**: `components/primitives.tsx` (Panel, AnimatedNumber, Sparkline, ProgressRing, TrendPill, LiveDot, EmptyState — todos respeitam `prefers-reduced-motion`) e `components/home/` (HeroToday, KpiRow, FlowChart com séries ligáveis/desligáveis, UpcomingSchedules, EventFeed com filtro por tipo, FleetPanel, SidePanels com alertas/confirmações/financeiro). `useDashboard` ganhou refresh manual, pausa do "ao vivo", proteção contra resposta fora de ordem e pausa com a aba em segundo plano.
  - **Removidos**: `components/StatsCards.tsx` e `components/DashboardCards.tsx` (código morto, sem nenhum import).
  - **Verificado no ambiente local** (Docker + seed, empresa Colégio Horizonte): payload da API conferido campo a campo contra o Postgres (alunos 2, ônibus 2, capacidade 70, dispositivos 1, série de 7 dias, 4 boletos vencidos somando R$ 1.510,00, 2 horários hoje já concluídos). Testados claro/escuro, sidebar recolhida, `Ctrl+K`, filtro do feed e toggle de séries do gráfico.
  - **Observação**: `frontend/.env.local` aponta `NEXT_PUBLIC_API_URL` para o IP da LAN (`192.168.0.103:4000`), então o painel precisa ser aberto por `http://192.168.0.103:3000` — em `localhost:3000` o login não completa porque o cookie de sessão vira cross-site.
